...
impl PermutationPlatform {
    fn new(failure_mode: Rc<Cell<bool>>) -> Self {
        Self {
            failure_mode,
...
fn mathematical_permutation_state_invariants_hold_across_large_sequences() {
    ...
    for run in 0..1_000 {
        let failure_mode = Rc::new(Cell::new(false));
        let mut controller = TimerController::new(
            PermutationPlatform::new(Rc::clone(&failure_mode)),
            Hns::new(5_000),
        );
        ...
        for _ in 0..step_count {
            ...
            match action {
                ...
                Step::ToggleFailure(fails) => {
                    failure_mode.set(fails);
                }
            }
