...
impl StartupRegistration for WindowsUserStartup {
    ...
    fn remove(&mut self) -> Result<(), StartupError> {
        #[cfg(windows)]
        {
            ...
            // SAFETY: key is a live HKEY from open_run_key and name points at a live nul terminated buffer.
...
fn register_validated(executable: &Path) -> Result<(), StartupError> {
    ...
    // SAFETY: key is a live HKEY from open_run_key and both buffers stay alive through the call.
    let status = unsafe {
        RegSetValueExW(
            key,
            name.as_ptr(),
...
#[cfg(windows)]
fn close_run_key(key: *mut std::ffi::c_void) -> Result<(), StartupError> {
    // SAFETY: key is a valid HKEY returned by open_run_key and closed exactly once.
    let status = unsafe { RegCloseKey(key) };
    if status != ERROR_SUCCESS {
        return Err(StartupError::CloseKey { raw_status: status });
    }
    Ok(())
}
...
fn open_run_key() -> Result<*mut std::ffi::c_void, StartupError> {
    ...
    // SAFETY: subkey points at a live nul terminated buffer and key and disposition receive valid out pointers.
    let status = unsafe {
        RegCreateKeyExW(
            HKEY_CURRENT_USER,
            subkey.as_ptr(),
...
mod tests {
    ...
    fn startup_error_variants_and_simulated_registry_status_preservation() {
        ...
        let close_error = StartupError::CloseKey { raw_status: 6 }; // ERROR_INVALID_HANDLE
        assert_eq!(close_error, StartupError::CloseKey { raw_status: 6 });
        assert_ne!(close_error, StartupError::RemoveValue { raw_status: 6 });
    }
