...
const LVN_GETDISPINFO: i32 = -177;
const LVSICF_NOINVALIDATEALL: usize = 0x0001;
...
#[repr(C)]
struct ListViewDisplayInfoNotification {
    header: NotifyHeader,
    item: ListViewItem,
}
...
struct DiagnosticGridRowBuffer {
...
impl DiagnosticGridRowBuffer {
    ...
    fn push(&mut self, row: DiagnosticGridRow) {
...
struct DiagnosticDisplayScratch {
    cells: Vec<u16>,
}
...
fn diagnostic_owner_data_with<R>(f: impl FnOnce(&mut DiagnosticOwnerData) -> R) -> Option<R> {
...
fn diagnostic_owner_buffer_with<R>(f: impl FnOnce(&mut DiagnosticGridRowBuffer) -> R) -> Option<R> {
...
fn diagnostic_displayed_item_count() -> usize {
