...
unsafe fn update_marquee_selection(app: &mut App, list: *mut c_void, band: &Rect) {
    ...
    let selected_indices = marquee_selected_indices(
        band,
        &row_rects,
        &app.diagnostic_ui.diagnostic_marquee_initial_selected,
        is_additive,
    );
    for index in 0..count {
...
unsafe fn handle_marquee_lbuttondown(hwnd: *mut c_void, l_param: isize) {
    ...
    app.diagnostic_ui.diagnostic_marquee_pending = true;
    app.diagnostic_ui.diagnostic_marquee_anchor = pt;
    app.diagnostic_ui.diagnostic_marquee_current = pt;
}
...
unsafe fn handle_marquee_pending_mousemove(hwnd: *mut c_void, l_param: isize) {
    ...
    let app = &mut *parent;
    if !app.diagnostic_ui.diagnostic_marquee_pending || app.diagnostic_ui.diagnostic_marquee_active
    {
        return;
    }
    let pt = point_from_lparam(l_param);
    ...
    if !marquee_drag_exceeded(
        app.diagnostic_ui.diagnostic_marquee_anchor,
        pt,
        cx_drag,
        cy_drag,
    ) {
    ...
    app.diagnostic_ui.diagnostic_marquee_pending = false;
    app.diagnostic_ui.diagnostic_marquee_active = true;
    SetCapture(hwnd);
    app.diagnostic_ui.diagnostic_marquee_current = pt;
    app.diagnostic_ui.diagnostic_marquee_initial_selected = list_selected_item_positions(hwnd);
    let band = points_to_rect(app.diagnostic_ui.diagnostic_marquee_anchor, pt);
...
unsafe fn handle_marquee_mousemove(hwnd: *mut c_void, l_param: isize) {
    ...
    let app = &mut *parent;
    if !app.diagnostic_ui.diagnostic_marquee_active {
        return;
    }
    let pt = point_from_lparam(l_param);
    draw_marquee_rect(
        hwnd,
        app.diagnostic_ui.diagnostic_marquee_anchor,
        app.diagnostic_ui.diagnostic_marquee_current,
    );
    app.diagnostic_ui.diagnostic_marquee_current = pt;
    draw_marquee_rect(
        hwnd,
...
unsafe fn handle_marquee_lbuttonup(hwnd: *mut c_void) {
    ...
    app.diagnostic_ui.diagnostic_marquee_active = false;
    app.diagnostic_ui
        .diagnostic_marquee_initial_selected
        .clear();
    if GetCapture() == hwnd {
        ReleaseCapture();
    }
...
pub(crate) unsafe extern "system" fn diagnostic_list_proc(
    ...
    let prev_proc = if !parent.is_null() {
        (*parent).diagnostic_ui.diagnostic_list_prev_proc
    } else {
        None
    };
    match message {
        ...
        WM_MOUSEMOVE => {
            ...
            if !left_button_held {
                if !parent.is_null()
                    && ((*parent).diagnostic_ui.diagnostic_marquee_pending
                        || (*parent).diagnostic_ui.diagnostic_marquee_active)
                {
                ...
                if !parent.is_null() && (*parent).diagnostic_ui.diagnostic_marquee_pending {
                    handle_marquee_pending_mousemove(hwnd, l_param);
                }
                if !parent.is_null() && (*parent).diagnostic_ui.diagnostic_marquee_active {
        ...
        WM_LBUTTONUP => {
            if !parent.is_null() && (*parent).diagnostic_ui.diagnostic_marquee_active {
                ...
                if !parent.is_null() {
                    (*parent).diagnostic_ui.diagnostic_marquee_pending = false;
                }
                if let Some(prev) = prev_proc {
                    CallWindowProcW(prev, hwnd, message, w_param, l_param)
                } else {
