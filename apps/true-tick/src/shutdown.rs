...
pub(crate) const fn shutdown_pump_bound_exceeded(monotonic_elapsed_ms: u64, bound_ms: u64) -> bool {
    monotonic_elapsed_ms >= bound_ms
}
...
mod tests {
    ...
    #[test]
    fn shutdown_pump_bound_is_enforced() {
        const BOUND_MS: u64 = 50;
        assert!(!shutdown_pump_bound_exceeded(0, BOUND_MS));
        assert!(!shutdown_pump_bound_exceeded(BOUND_MS - 1, BOUND_MS));
        assert!(shutdown_pump_bound_exceeded(BOUND_MS, BOUND_MS));
        assert!(shutdown_pump_bound_exceeded(BOUND_MS + 1, BOUND_MS));
    }

    #[test]
    fn pump_bound_uses_monotonic_elapsed() {
        const BOUND_MS: u64 = 50;
        let start = std::time::Instant::now();
        let elapsed_ms = u64::try_from(start.elapsed().as_millis()).unwrap_or(u64::MAX);
        assert!(!shutdown_pump_bound_exceeded(elapsed_ms, BOUND_MS));
        assert!(shutdown_pump_bound_exceeded(BOUND_MS, BOUND_MS));
    }
}
