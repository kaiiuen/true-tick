...
pub(crate) fn power_reconciliation(
    ...
    match power {
        ...
        PowerState::Battery | PowerState::BatterySaver if !battery_lockout => match ownership {
            OwnershipState::Owned => PowerReconciliation::PreserveOwned,
            OwnershipState::Uncertain => PowerReconciliation::PreserveUncertain,
            OwnershipState::Released if automatic => PowerReconciliation::Acquire,
            OwnershipState::Released => PowerReconciliation::ShowStopped,
        },
...
pub(crate) const fn battery_lockout_label(enabled: bool) -> &'static str {
    if enabled {
        "Battery lockout: On"
    } else {
        "Battery lockout: Off"
...
pub(crate) const BATTERY_LOCKOUT_ON_COMMAND_ID: usize = 1027;
pub(crate) const BATTERY_LOCKOUT_OFF_COMMAND_ID: usize = 1028;
...
pub(crate) const fn menu_description(command_id: usize) -> Option<&'static str> {
    match command_id {
        ...
        BATTERY_LOCKOUT_ON_COMMAND_ID | BATTERY_LOCKOUT_OFF_COMMAND_ID => {
            Some("Restrict timing requests on DC power")
        }
        1060 => Some("Clear all data and restart"),
...
pub(crate) const fn menu_action_keeps_open(command_id: usize) -> bool {
    matches!(
        ...
        1001 | 1002 | 1005
            ..=1010
                | BATTERY_LOCKOUT_ON_COMMAND_ID
                | BATTERY_LOCKOUT_OFF_COMMAND_ID
...
pub(crate) const fn menu_command_is_enabled_with_pause(
    ...
    match command_id {
        ...
        BATTERY_LOCKOUT_ON_COMMAND_ID => !battery_lockout,
        BATTERY_LOCKOUT_OFF_COMMAND_ID => battery_lockout,
        LOGS_COMMAND_ID | GITHUB_COMMAND_ID | SCHEDULE_PRESETS_COMMAND_ID | 1004 | 1060 => true,
        START_IN_1_COMMAND_ID
        | START_IN_5_COMMAND_ID
        | START_IN_15_COMMAND_ID
        | START_IN_30_COMMAND_ID
...
pub(crate) fn settings_menu_items(
    ...
    vec![
        ...
        MenuItem {
            label: battery_lockout_label(battery_lockout).to_owned(),
            enabled: true,
            command_id: None,
        },
    ]
...
mod tests {
    ...
    fn auto_time_off_on_ac_without_ownership_shows_stopped() {
        assert_eq!(
            power_reconciliation(false, PowerState::Ac, OwnershipState::Released, true),
    ...
    fn auto_time_off_battery_to_ac_waits_for_manual_start() {
        assert_eq!(
            power_reconciliation(false, PowerState::Battery, OwnershipState::Released, true),
            PowerReconciliation::ReleaseBlocked
        );
        let reconciliation =
            power_reconciliation(false, PowerState::Ac, OwnershipState::Released, true);
    ...
    fn auto_time_on_battery_to_ac_acquires_when_released() {
        assert_eq!(
            power_reconciliation(true, PowerState::Battery, OwnershipState::Released, true),
            PowerReconciliation::ReleaseBlocked
        );
        assert_eq!(
            power_reconciliation(true, PowerState::Ac, OwnershipState::Released, true),
            PowerReconciliation::Acquire
        );
    }
    ...
    fn owned_ac_timing_is_preserved_when_auto_time_is_off() {
        assert_eq!(
            power_reconciliation(false, PowerState::Ac, OwnershipState::Owned, true),
    ...
    fn battery_releases_regardless_of_auto_time() {
        assert_eq!(
            power_reconciliation(true, PowerState::Battery, OwnershipState::Owned, true),
            PowerReconciliation::ReleaseBlocked
        );
        assert_eq!(
            power_reconciliation(false, PowerState::Battery, OwnershipState::Owned, true),
            PowerReconciliation::ReleaseBlocked
        );
    }
    ...
    fn unknown_power_releases_conservatively() {
        assert_eq!(
            power_reconciliation(true, PowerState::Unknown, OwnershipState::Owned, true),
            PowerReconciliation::ReleaseBlocked
        );
        assert_eq!(
            power_reconciliation(true, PowerState::Unknown, OwnershipState::Owned, false),
            PowerReconciliation::ReleaseBlocked
        );
    }

    #[test]
    fn battery_and_saver_preserve_and_acquire_when_lockout_is_off() {
        for power in [PowerState::Battery, PowerState::BatterySaver] {
            assert_eq!(
                power_reconciliation(true, power, OwnershipState::Owned, false),
                PowerReconciliation::PreserveOwned
            );
            assert_eq!(
                power_reconciliation(true, power, OwnershipState::Uncertain, false),
                PowerReconciliation::PreserveUncertain
            );
            assert_eq!(
                power_reconciliation(true, power, OwnershipState::Released, false),
                PowerReconciliation::Acquire
            );
            assert_eq!(
                power_reconciliation(false, power, OwnershipState::Released, false),
                PowerReconciliation::ShowStopped
            );
        }
    }
    ...
    fn repeated_commands_follow_the_current_state_without_duplicate_actions() {
        ...
        assert!(menu_command_is_enabled(
            1001, status, false, false, true, true
        ));
        ...
        assert!(menu_command_is_enabled(
            1002, status, false, false, true, true
        ));
        ...
        assert!(menu_command_is_enabled(
            1007, status, false, automatic, true, true,
        ));
        automatic = true;
        assert!(!menu_command_is_enabled(
            1007, status, false, automatic, true, true,
        ...
        assert!(menu_command_is_enabled(
            1008, status, false, automatic, true, true,
        ));
        automatic = false;
        assert!(!menu_command_is_enabled(
            1008, status, false, automatic, true, true,
