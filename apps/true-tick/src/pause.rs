...
impl DurationPreset {
    ...
    pub fn parse(text: &str) -> Result<Self, &'static str> {
        ...
        {
            let micros = parse_decimal_micros(&lower)?;
            let total_micros = micros.checked_mul(60u64).ok_or("invalid duration number")?;
            return Self::from_micros_rounded(total_micros);
        }
        ...
        loop {
            ...
            let unit_micros = match unit.as_str() {
                "s" | "sec" | "secs" | "second" | "seconds" => 1_000_000u64,
                "m" | "min" | "mins" | "minute" | "minutes" => 60 * 1_000_000,
                "h" | "hr" | "hrs" | "hour" | "hours" => 3600 * 1_000_000,
                "" => return Err("expected a unit after the number"),
                _ => return Err("unknown duration unit"),
            };
            let segment_micros = if seen_dot {
                let micros = parse_decimal_micros(&number_text)?;
                micros
                    .checked_mul(unit_micros)
                    .map(|scaled| scaled / 1_000_000)
                    .ok_or("invalid duration number")?
            } else {
                let value: u64 = number_text.parse().map_err(|_| "invalid duration number")?;
                value
                    .checked_mul(unit_micros)
                    .ok_or("invalid duration number")?
            };
            let segment_seconds = segment_micros
                .checked_add(500_000)
                .map(|shifted| shifted / 1_000_000)
                .ok_or("invalid duration number")?;
    ...
    fn from_micros_rounded(micros: u64) -> Result<Self, &'static str> {
        let shifted = micros
            .checked_add(500_000)
            .ok_or("invalid duration number")?;
        let total_seconds = shifted / 1_000_000;
        if total_seconds < MIN_PRESET_SECONDS as u64 || total_seconds > MAX_PRESET_SECONDS as u64 {
            return Err("preset must be between 10 seconds and 24 hours");
        }
        Self::new(total_seconds as u32)
    }
...
fn parse_decimal_micros(text: &str) -> Result<u64, &'static str> {
    ...
    let integer_part = if integer_text.is_empty() {
        if fraction_text.is_some() {
            0u64
        } else {
            return Err("invalid duration number");
        }
    } else {
        match integer_text.parse::<u64>() {
            Ok(value) => value,
            Err(_) => return Err("invalid duration number"),
        }
    };
    let fractional_micros = match fraction_text {
        Some(fraction) => {
            if fraction.len() > 6 {
                return Err("invalid duration number");
            }
...
pub(crate) fn acquisition_is_allowed(
    paused: bool,
    automatic: bool,
    power: PowerState,
    battery_lockout: bool,
) -> bool {
    !paused
        && automatic
        && decide(PolicyInput {
            ...
            power,
            battery_lockout_enabled: battery_lockout,
        })
        .status
            == Status::Requested
}
...
mod tests {
    ...
    fn pause_suppresses_acquisition_for_all_power_states() {
        for power in [
            ...
            assert!(!acquisition_is_allowed(true, true, power, true));
        }
        assert!(acquisition_is_allowed(false, true, PowerState::Ac, true));
        assert!(!acquisition_is_allowed(false, false, PowerState::Ac, true));
        assert!(!acquisition_is_allowed(
            false,
            true,
            PowerState::Battery,
            true
    ...
    #[test]
    fn parse_decimal_micros_accepts_one_point_five_hours_as_5400_seconds() {
        assert_eq!(DurationPreset::parse("1.5h").unwrap().seconds(), 5400);
    }

    #[test]
    fn parse_decimal_micros_accepts_zero_point_one_seconds() {
        assert_eq!(DurationPreset::parse("10.0s").unwrap().seconds(), 10);
        assert_eq!(DurationPreset::parse("59.5s").unwrap().seconds(), 60);
    }

    #[test]
    fn parse_decimal_micros_rejects_multiple_dots() {
        assert!(DurationPreset::parse("1.2.3h").is_err());
        assert!(DurationPreset::parse("1.2.3").is_err());
    }

    #[test]
    fn parse_decimal_micros_rejects_more_than_six_fraction_digits() {
        assert!(DurationPreset::parse("1.0000001h").is_err());
        assert!(DurationPreset::parse("0.0000001m").is_err());
    }

    #[test]
    fn parse_rejects_overflowing_hour_values() {
        assert!(DurationPreset::parse("99999h").is_err());
        assert!(DurationPreset::parse("99999.5h").is_err());
    }

    #[test]
    fn parse_decimal_inputs_yield_exact_integer_results() {
        assert_eq!(DurationPreset::parse("0.166666m").unwrap().seconds(), 10);
        assert_eq!(DurationPreset::parse("1.000001h").unwrap().seconds(), 3600);
        assert_eq!(DurationPreset::parse("2.5m").unwrap().seconds(), 150);
        assert_eq!(DurationPreset::parse("1.5h").unwrap().seconds(), 5400);
    }
