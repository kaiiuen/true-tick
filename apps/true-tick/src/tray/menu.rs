...
use super::{
    ...
    popup_track_flags, quit_decision, reset_cleanup, schedule_duration_action,
    schedule_preset_action, set_auto_resume, set_automatic, set_battery_lockout, set_startup,
    show_quit_warning, show_shutdown_warning, taskbar_edge, wide, App, AppendMenuW,
    CreatePopupMenu, CreateWindowExW, DestroyMenu, DestroyWindow, DiagnosticOutcome,
    DiagnosticSource, ExitProcess, GetCursorPos, GetLastError, GetMenuItemCount, GetMenuStringW,
    GetModuleHandleW, GetSystemMetrics, KillTimer, MessageBoxW, ModifyMenuW, NativeResult, Point,
    PopupMenuHandles, PostQuitMessage, QuitDecision, QuitDialogDecision, Rect, SendMessageW,
    SetForegroundWindow, SetTimer, ToolInfo, TrackPopupMenu, ID_AUTOMATIC_OFF, ID_AUTOMATIC_ON,
    ID_AUTO_RESUME_OFF, ID_AUTO_RESUME_ON, ID_BATTERY_LOCKOUT_OFF, ID_BATTERY_LOCKOUT_ON, ID_QUIT,
    ID_RESET, ID_START, ID_STARTUP_OFF, ID_STARTUP_ON, ID_STOP, MB_ICONINFORMATION, MB_ICONWARNING,
    MB_OK, POPUP_REFRESH_INTERVAL_MS, POPUP_REFRESH_TIMER_ID, TTF_ABSOLUTE, TTF_IDISHWND,
    TTF_TRACK, TTM_ADDTOOLW, TTM_TRACKACTIVATE, TTM_TRACKPOSITION, TTM_UPDATETIPTEXTW,
    TTS_ALWAYSTIP, TTS_NOPREFIX, WS_EX_TOPMOST, WS_POPUP,
};
...
pub(crate) const TPM_RIGHTALIGN: u32 = 0x0008;
pub(crate) const TPM_BOTTOMALIGN: u32 = 0x0020;
...
pub(crate) const SETTINGS_MENU_BATTERY_LOCKOUT_POSITION: usize = 3;
#[allow(dead_code)]
pub(crate) const SETTINGS_MENU_RESET_POSITION: usize = 5;

/// How the popup anchor was chosen, for diagnostics.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum MenuAnchorSource {
    /// `Shell_NotifyIconGetRect` returned the on-screen icon bounds.
    IconRect,
    /// The icon rect query failed, so the anchor follows the cursor.
    CursorFallback,
}

/// Resolved popup anchor: the point handed to `TrackPopupMenu` and the
/// alignment flags that decide which corner of the menu sits on it.
pub(crate) struct MenuAnchor {
    pub(crate) point: Point,
    pub(crate) flags: u32,
    pub(crate) source: MenuAnchorSource,
}

/// `NOTIFYICONIDENTIFIER` for `Shell_NotifyIconGetRect`. The tray icon was
/// registered without a GUID, so `guid_item` stays `GUID_NULL` and the icon is
/// matched by `h_wnd` plus `u_id`.
#[repr(C)]
pub(crate) struct NotifyIconIdentifier {
    cb_size: u32,
    h_wnd: *mut c_void,
    u_id: u32,
    guid_item: [u8; 16],
...
extern "system" {
    fn Shell_NotifyIconGetRect(
        ...
        icon_location: *mut Rect,
    ) -> i32;
}

/// Asks the shell for the tray icon's screen rect. Returns `None` when the
/// call fails, which covers older Windows builds and icons living in the
/// hidden-icon overflow flyout.
fn query_icon_rect(hwnd: *mut c_void) -> Option<Rect> {
...
/// Picks the popup anchor. When the icon rect is available the menu anchors to
/// the icon's top-left corner with `TPM_BOTTOMALIGN | TPM_LEFTALIGN` so it
/// grows upward and rightward. Otherwise it falls back to the cursor point
/// plus taskbar-edge alignment so nothing regresses.
pub(crate) fn menu_anchor(icon_rect: Option<Rect>, edge: Option<u32>, cursor: Point) -> MenuAnchor {
    if let Some(rect) = icon_rect {
        MenuAnchor {
            ...
            source: MenuAnchorSource::IconRect,
        }
    } else {
        MenuAnchor {
            point: cursor,
            flags: popup_track_flags(edge, TPM_LEFTALIGN, TPM_TOPALIGN),
            source: MenuAnchorSource::CursorFallback,
        }
    }
}
...
pub(crate) unsafe fn show_menu(hwnd: *mut c_void, app: &mut App) {
    ...
    // Only clamp against the primary work area when cursor is inside the primary coordinate quadrant
    // to preserve negative virtual coordinates on multi-monitor setups.
    if work_width > 0 && work_height > 0 && cursor.x >= 0 && cursor.y >= 0 {
        cursor = clamp_menu_anchor(
            cursor,
            Rect {
                left: 0,
                top: 0,
    ...
    let anchor = menu_anchor(query_icon_rect(hwnd), taskbar_edge(), cursor);
    app.record(
        "tray.menu.anchor",
        format!(
            "source={:?} x={} y={}",
            anchor.source, anchor.point.x, anchor.point.y
        ),
    );
    loop {
        let (menu, track_target) = if app.settings_submenu_open {
            ...
            let battery_lockout_id = if app.config.battery_lockout {
                ID_BATTERY_LOCKOUT_OFF
            } else {
                ID_BATTERY_LOCKOUT_ON
            };
            ...
            ok &= append_menu_checked(app, submenu, MF_STRING, ID_RESET, wide("Reset").as_ptr());
            if !ok {
                DestroyMenu(submenu);
                break;
            }
        ...
        let command = TrackPopupMenu(
            track_target,
            anchor.flags | TPM_RIGHTBUTTON | TPM_RETURNCMD,
            anchor.point.x,
            anchor.point.y,
            0,
            hwnd,
...
pub(crate) unsafe fn refresh_popup_menu(app: &mut App) {
    ...
    if let Some(settings_menu) = handles.settings {
        ...
        let _ = ModifyMenuW(
            settings_menu,
            SETTINGS_MENU_BATTERY_LOCKOUT_POSITION,
            MF_BYPOSITION | MF_STRING,
            battery_lockout_id,
            wide(&items[3].label).as_ptr(),
        );
...
pub(crate) unsafe fn handle_menu_command(hwnd: *mut c_void, app: &mut App, command: usize) -> bool {
    ...
    let action_name = match command {
        ...
        ID_BATTERY_LOCKOUT_ON => "battery_lockout_on",
        ID_BATTERY_LOCKOUT_OFF => "battery_lockout_off",
        ID_RESET => "reset",
        ID_QUIT => "quit",
        _ => {
    ...
    match command {
        ...
        ID_BATTERY_LOCKOUT_ON => set_battery_lockout(app, true),
        ID_BATTERY_LOCKOUT_OFF => set_battery_lockout(app, false),
        ID_RESET => return execute_reset(hwnd, app),
        ID_QUIT => {
...
/// Settings Reset handler. Wipes the log directory contents, the config file,
/// and the session tombstone, persists the `settings.reset.executed` event into
/// a fresh log file, then relaunches the executable and terminates this
/// process. Returns `false` only when the relaunch fails so the menu loop can
/// close normally while the app stays alive.
unsafe fn execute_reset(hwnd: *mut c_void, app: &mut App) -> bool {
    app.record("settings.reset.executed", "source=tray");
    let report = reset_cleanup(&app.log_directory, &app.state_directory, &app.config_path);
    app.record(
        "settings.reset.cleanup",
        format!(
            "log_entries_removed={} log_entries_failed={} config_removed={} session_marker_removed={}",
            report.log_entries_removed,
            report.log_entries_failed,
            report.config_removed,
            report.session_marker_removed
        ),
    );
    ...
    MessageBoxW(
        hwnd,
        wide("All data cleared. Restarting...").as_ptr(),
        wide("True™ Tick").as_ptr(),
    ...
    let executable = match std::env::current_exe() {
        ...
        Err(error) => {
            app.record_with_outcome(
                "settings.reset.relaunch.error",
                format!("stage=current_exe error={error}"),
                DiagnosticOutcome::Failed,
            );
    ...
    match spawned {
        Ok(_child) => {
            app.record("settings.reset.relaunch", "result=spawned");
            ExitProcess(0);
        }
        Err(error) => {
            app.record_with_outcome(
                "settings.reset.relaunch.error",
                format!("stage=spawn error={error}"),
                DiagnosticOutcome::Failed,
            );
            app.finish_operation(DiagnosticOutcome::Failed);
            false
...
mod tests {
    ...
    fn settings_menu_positions_and_commands_match_settings_contract() {
        ...
        assert_eq!(SETTINGS_MENU_BATTERY_LOCKOUT_POSITION, 3);
        assert_eq!(SETTINGS_MENU_RESET_POSITION, 5);
        assert_eq!(ID_AUTO_RESUME_ON, 1009);
        assert_eq!(ID_AUTO_RESUME_OFF, 1010);
        assert_eq!(ID_BATTERY_LOCKOUT_ON, 1027);
        assert_eq!(ID_BATTERY_LOCKOUT_OFF, 1028);
        assert_eq!(ID_RESET, 1060);
    }

    #[test]
    fn popup_track_flags_follow_all_four_taskbar_edges() {
        use crate::tray::{ABE_BOTTOM, ABE_LEFT, ABE_RIGHT, ABE_TOP};
        let base = TPM_LEFTALIGN | TPM_TOPALIGN | TPM_RIGHTBUTTON | TPM_RETURNCMD;
        assert_eq!(
            popup_track_flags(None, TPM_LEFTALIGN, TPM_TOPALIGN) | TPM_RIGHTBUTTON | TPM_RETURNCMD,
            base
        );
        for edge in [Some(ABE_LEFT), Some(ABE_TOP)] {
            let flags = popup_track_flags(edge, TPM_LEFTALIGN, TPM_TOPALIGN)
                | TPM_RIGHTBUTTON
                | TPM_RETURNCMD;
            assert_eq!(flags, base);
        }
        let right = popup_track_flags(Some(ABE_RIGHT), TPM_LEFTALIGN, TPM_TOPALIGN)
            | TPM_RIGHTBUTTON
            | TPM_RETURNCMD;
        assert_eq!(
            right,
        ...
        let bottom = popup_track_flags(Some(ABE_BOTTOM), TPM_LEFTALIGN, TPM_TOPALIGN)
            | TPM_RIGHTBUTTON
            | TPM_RETURNCMD;
        assert_eq!(
            bottom,
        ...
        let unknown = popup_track_flags(Some(0xffff), TPM_LEFTALIGN, TPM_TOPALIGN)
            | TPM_RIGHTBUTTON
            | TPM_RETURNCMD;
    ...
    #[test]
    fn menu_anchor_prefers_icon_rect_and_grows_upward_from_top_left() {
        let rect = Rect {
            left: 1880,
            top: 1040,
            right: 1912,
        ...
        let cursor = Point { x: 100, y: 200 };
        let anchor = menu_anchor(Some(rect), Some(crate::tray::ABE_BOTTOM), cursor);
        assert_eq!(anchor.source, MenuAnchorSource::IconRect);
        assert_eq!(anchor.point, Point { x: 1880, y: 1040 });
        assert_eq!(anchor.flags, TPM_BOTTOMALIGN | TPM_LEFTALIGN);
        assert_eq!(anchor.flags & TPM_RIGHTALIGN, 0);
    }

    #[test]
    fn menu_anchor_falls_back_to_cursor_with_taskbar_edge_flags() {
        let cursor = Point { x: 640, y: 1000 };
        let anchor = menu_anchor(None, Some(crate::tray::ABE_RIGHT), cursor);
        assert_eq!(anchor.source, MenuAnchorSource::CursorFallback);
        assert_eq!(anchor.point, cursor);
        assert_eq!(anchor.flags, TPM_RIGHTALIGN | TPM_TOPALIGN);
        let anchor = menu_anchor(None, None, cursor);
        assert_eq!(anchor.flags, TPM_LEFTALIGN | TPM_TOPALIGN);
    }
    ...
    #[test]
    fn reset_cleanup_deletes_logs_config_and_marker_and_records_event() {
        use std::sync::Arc;
        ...
        let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("target")
            .join("test-tmp")
            .join(format!("settings-reset-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        let log_directory = root.join("Data").join("logs");
        let state_directory = root.join("Data").join("state");
        std::fs::create_dir_all(&log_directory).expect("log directory created");
        std::fs::create_dir_all(&state_directory).expect("state directory created");
        std::fs::write(log_directory.join("true-tick-2026-09-20.csv"), "Row\n1,x\n")
            .expect("csv written");
        std::fs::write(state_directory.join("session.state"), "state=running\n")
            .expect("marker written");
        let config_path = root.join("true-tick.toml");
        std::fs::write(&config_path, "automatic = true\n").expect("config written");

        let store = Arc::new(DiagnosticStore::new(16));
        let mut app = crate::tray::tests::test_app(store.clone());
        app.log_directory = log_directory.clone();
        app.state_directory = state_directory.clone();
        app.config_path = config_path.clone();

        app.record("settings.reset.executed", "source=tray");
        let report = reset_cleanup(&app.log_directory, &app.state_directory, &app.config_path);

        assert!(report.session_marker_removed);
        assert!(report.config_removed);
        assert!(report.log_entries_removed >= 1);
        assert_eq!(report.log_entries_failed, 0);
        assert!(!crate::session::session_marker_path(&state_directory).exists());
        assert!(!config_path.exists());
        assert!(log_directory.exists());
        assert!(std::fs::read_dir(&log_directory)
        ...
        let event = events
            .iter()
            .find(|event| event.name == "settings.reset.executed")
            .expect("reset event recorded");
        assert!(event.details.contains("source=tray"));
