...
The diagnostic window has an explicit summary control and a native `SysListView32` report control. Its columns are `Sequence`, `Elapsed`, `Operation`, `Parent`, `Correlation`, `Phase`, `Source`, `Outcome`, `Event`, and `Details`. Each existing `DiagnosticStore` snapshot is converted from typed `DiagnosticEvent` values. The native path inserts the first cell with `LVM_INSERTITEMW` on the list HWND and fills remaining cells with `LVM_SETITEMTEXTW`, whose synchronous control messages copy the bounded UTF-16 text. List creation failure, negative `LVM_INSERTITEMW`, failed `LVM_SETITEMTEXTW`, and the optional post-refresh item count are recorded with bounded details, native return values, and raw Win32 status values. The bounded Details cell preserves raw HNS, typed NTSTATUS and Win32 last-error values, timing observations, schedule generations, and operation lineage. Refreshes are posted and coalesced on the diagnostic window UI message loop. The report is read-only, session-local, newest-retained, capped at 512 events, and marked when truncation occurs. A failed timing observation is `Unknown` rather than stale current data. Closing Logs does not change timer state.

The diagnostic window also owns explicit `Rows:`, range edit, `Copy`, `Export`, and validation message HWNDs. Summary, toolbar, and grid layout is recalculated from the current client size and DPI. The range parser operates on the current `DiagnosticStore` snapshot and uses chronological 1-based visible report rows. Empty input and `all` select all retained rows. A single number selects one row and `start-end` is inclusive. Invalid, reversed, zero, negative, and out-of-range values leave both actions disabled and record bounded validation diagnostics. A refreshed snapshot revalidates the input without changing timer ownership.

TSV conversion starts from the same ten typed grid columns used by `SysListView32`. Cells are sanitized again before output, control layout is removed, each field remains bounded, and output rows are capped by the hard diagnostic retention bound. Copy uses `OpenClipboard`, movable global memory, `SetClipboardData` with `CF_UNICODETEXT`, and guaranteed cleanup on failure. Successful clipboard ownership is transferred to Windows only after the data is ready. Export uses `GetSaveFileNameW`, treats a zero extended dialog error as user cancellation, writes UTF-8 bytes to a same-directory temporary path, and uses `MoveFileExW` with replacement and write-through flags. The selected path is not recorded in diagnostics. Copy and export emit range, row count, format, operation, correlation, and result context through `diagnostic.range.parsed`, `diagnostic.copy.request`, `diagnostic.copy.result`, `diagnostic.export.request`, and `diagnostic.export.result` events. Native clipboard, dialog, and file failures retain bounded raw Win32 values.
