...
The diagnostic window also owns explicit `Rows:`, selection summary, range edit, `Copy`, `Export`, and validation message HWNDs. Summary, toolbar, and grid layout is recalculated from the current client size and DPI. The range parser operates on the current `DiagnosticStore` snapshot and uses retained 1-based row positions. Empty input and `all` select all retained rows. A single number selects one row and `start-end` is inclusive. For example, `353-354` selects retained rows that can display event sequences `753-754`. Invalid, reversed, zero, negative, and out-of-range values leave both actions disabled and record bounded validation diagnostics. A refreshed snapshot preserves selected event sequences when they remain retained, updates their Row values, and visibly resets and logs the selection when retention removed them.

TSV conversion starts from the same eleven typed grid columns used by `SysListView32`, with `Row` before `Sequence`. Cells are sanitized again before output, control layout is removed, each field remains bounded, and output rows are capped by the hard diagnostic retention bound. Copy uses `OpenClipboard`, movable global memory, `SetClipboardData` with `CF_UNICODETEXT`, and guaranteed cleanup on failure. Successful clipboard ownership is transferred to Windows only after the data is ready. Export uses `GetSaveFileNameW`, treats a zero extended dialog error as user cancellation, writes UTF-8 bytes to a same-directory temporary path, and uses `MoveFileExW` with replacement and write-through flags. The selected path is not recorded in diagnostics. Copy and export emit range, row count, format, operation, correlation, and result context through `diagnostic.range.parsed`, `diagnostic.copy.request`, `diagnostic.copy.result`, `diagnostic.export.request`, and `diagnostic.export.result` events. Native clipboard, dialog, and file failures retain bounded raw Win32 values.
