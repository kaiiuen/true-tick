...
The summary is a read-only `STATIC` region with stable wrapped text. It has no vertical scroll style, automatic vertical scroll style, or multiline edit behavior. The base summary height is 220 logical pixels. The toolbar is a computed one-row flow from fixed control widths, eight DPI-scaled gaps, margins, and a 140 logical pixel flexible message minimum. Its 96 DPI client minimum is 916 pixels. The client minimum height is 352 pixels at 96 DPI, which reserves the summary, toolbar, and a 96 pixel grid viewport.
...
`WM_SIZE` performs layout and Details viewport fitting only. It does not take a snapshot, rebuild rows, or auto-fit content. `WM_DPICHANGED` applies the suggested rectangle, reapplies the GUI font, recalculates layout, and fits the Details column to the viewport while preserving horizontal scroll when possible. Content auto-fit occurs only on initial population or a material snapshot generation.
