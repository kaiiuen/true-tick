...
Deferred positioning logs begin, per-control, and end failures, then falls back to individual positioning for every child. Creation failure destroys all partial children and returns `-1`. Reuse checks the parent and all required children before showing. Parent destruction clears all child handles, selections, refresh flags, and layout state together.
