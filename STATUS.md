...
The report grid has normal Windows multi-row selection, full-row selection, visible selection, gridlines, and normal mouse drag selection. Its toolbar summary says `Selected: N rows`. Selected rows are copied or exported in chronological retained order. `Row` remains the current retained position and `Sequence` remains the event identity.
