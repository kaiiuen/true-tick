...
The report grid has normal Windows multi-row selection, full-row selection, visible selection, gridlines, and native mouse drag selection. Its one-row toolbar summary says `Selected: N rows`. Selected rows are copied or exported in chronological retained order. `Row` remains the current retained position and `Sequence` remains the event identity. Headers and visible content are auto-fitted with native ListView column-width messages after refresh and resize. Fixed columns use DPI-scaled bounds, Details fills remaining client width, and horizontal scrolling remains available when needed. Edit text uses a system GUI font and a minimum control height. System-color erase and paint handling prevents a black widened area and preserves high-contrast behavior where Windows provides it.
