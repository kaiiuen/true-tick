...
The diagnostic layout contract is now stable. The summary is a non-scrolling read-only `STATIC` region with a DPI-scaled 220 logical pixel base height. It is separate from the report grid and has no multiline edit, vertical scroll, or automatic vertical scroll style. The toolbar is one computed horizontal row. At 96 DPI the fixed controls, eight gaps, margins, and flexible message minimum require a 916 pixel client width. The minimum client height is 352 pixels.
...
`WM_SIZE` only lays out controls and fits Details to the current viewport. `WM_DPICHANGED` uses the suggested rectangle, reapplies the GUI font, recalculates the layout, and preserves horizontal scroll where possible. It does not snapshot data, rebuild rows, or content auto-fit. Auto-fit is limited to initial population and material snapshot changes. One refresh uses one snapshot and permits one bounded follow-up for refresh-generated records. The session remains capped at 512 events.
