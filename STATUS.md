...
Logs now shows a concise summary and a read-only native `SysListView32` grid. The list and tooltip classes are initialized once with `InitCommonControlsEx` before controls are created, and the embedded v6 manifest remains required. The columns are `Sequence`, `Elapsed`, `Operation`, `Parent`, `Correlation`, `Phase`, `Source`, `Outcome`, `Event`, and `Details`. Rows come from the existing `DiagnosticStore` snapshot through typed `DiagnosticEvent` conversion. List creation failure, negative `LVM_INSERTITEMW`, failed `LVM_SETITEMTEXTW`, and the optional row or item count after refresh are recorded with bounded details, native return values, and raw Win32 status values. Details preserve raw HNS, operation lineage, pause and schedule generations, timing observations, NTSTATUS, and Win32 error values. Refresh is coalesced through the UI message loop. The local session retains at most 512 newest events and one truncation marker. It is not disk telemetry. Unknown or stale timing evidence is displayed as `Unknown`.

The Logs window now has a compact `Rows:` toolbar between the summary and grid. The input accepts empty or `all` for all chronological retained session rows, one visible 1-based row number, or an inclusive `start-end` range such as `12-24`. Whitespace is trimmed. Invalid, reversed, zero, negative, and out-of-range values show a short message, create a bounded diagnostic validation event, and keep Copy and Export disabled. Retention truncation means earlier chronological rows may no longer exist.

Copy and Export use the current bounded snapshot only. Copy places a sanitized, bounded TSV with the ten report columns on the standard Windows clipboard. Export opens the standard Save dialog with `true-tick-log.tsv` as the default filename and writes a UTF-8 TSV through a same-directory temporary replacement. Dialog cancellation is logged without error noise. Native clipboard, dialog, and file failures show a short window warning and retain raw native status values in session diagnostics. The toolbar creates explicit HWND controls, follows normal DPI and window resizing, and does not acquire, release, pause, schedule, or otherwise alter timer state. No automatic files, telemetry, credentials, secrets, raw pointers, or uncontrolled logs are produced.
