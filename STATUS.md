...
The toolbar also carries a `Search:` box between the category filter combo and the transfer group. The search filters grid rows case insensitively over the event name and the details text and composes with the active category filter, so a visible row must satisfy both the selected `EventCategory` and the search substring. The HUD `Showing X of Y retained rows` count reflects the composed filter. `Export all` ignores the grid selection, the transfer range, the category filter, and the search text and always exports the complete unfiltered snapshot.
...
The diagnostic layout contract is now stable. The summary is a restrained native HUD with a larger bold lifecycle-derived State line and concise key/value fields for Effective timing, Ownership, Power, Startup, Running duration, Next action, retained event history, visible row count, and chain integrity. Native etched separators separate the HUD from the one-row toolbar and the toolbar from the grid. The HUD uses read-only non-scrolling `STATIC` controls with a DPI-scaled 140 logical pixel base height. The larger bold State line occupies 32 of those pixels and the key/value block occupies the remainder. The HUD renders one compact five-line telemetry block with `|` separators instead of an eight-line vertical dump. Line one carries Effective, Ownership, and Power. Line two carries Startup, Running, and Next. Line three carries the History label, the `Showing X of Y retained rows` count, and the chain integrity status. Line four carries the operating slot and the resolved portable root. Line five is the About identity line with the branded product name, the Cargo package version, the creator, and the GitHub link. The chain field runs `verify_event_chain` over the current snapshot and shows `Chain: OK` when every entry hash and link verifies, or `Chain: Error at #<index>` naming the first offending row when any event was modified. At 96 DPI the toolbar client minimum is 1190 pixels and the minimum client height is 276 pixels, which is the 140 pixel HUD plus a 2 pixel separator plus the 36 pixel toolbar plus a 2 pixel separator plus the 96 pixel grid viewport. The compact default opens at a 1280x520 client area at 96 DPI with the outer size clamped to the primary work area. Outer tracking limits are derived from those client minimums with DPI-aware frame metrics, so the controls retain their intended client space.

Diagnostic layout positions every control with a direct `SetWindowPos` call rather than one `BeginDeferWindowPos` batch. The pass checks `IsWindow` and positive rectangle dimensions per control, records `diagnostic.layout.skipped` with a bounded reason when a control is skipped, records `diagnostic.layout.failure` with the `SetWindowPos` stage and the raw Win32 status for a hard failure, and stops at the first hard failure. Removing the deferred batch eliminates the recurring `ERROR_INVALID_PARAMETER` layout errors on initial frame layout and on repaint passes. Creation failure destroys all partial children and returns `-1`. Successful creation assigns every required child HWND, including both etched separators, before initial layout and showing. Reuse checks the parent and all required children before showing. Parent destruction clears all child handles, selections, refresh flags, and layout state together.
...
## Session marker, self containment, and feedback surfaces

True Tick now records its own session boundary so an unclean exit is visible on the next launch. At startup the app resolves the log directory, reads `session.state` from it, and classifies the previous session. The marker holds `running` while the process is alive and `clean` when the shutdown path completes, and the same directory holds the rolling daily logs. The resolution order is `Data/logs` under the portable root for a slot layout and `logs` beside the executable otherwise. A previous marker of `running` or any unrecognized content classifies as an unclean shutdown, `clean` classifies as a clean exit, and an absent marker classifies as a first run. An unclean shutdown records `lifecycle.unclean_shutdown` with the previous marker text and shows a non-blocking warning that the last session did not shut down cleanly. A clean exit records `lifecycle.clean_start` with `previous_marker=clean`, and a first run records `lifecycle.clean_start` with `previous_marker=absent`. Marker write failures record `lifecycle.session_marker.error` with `result=running_write_failed` or `result=clean_write_failed` and never change timer state. Session classification and the marker round trip have deterministic unit coverage.

The self containment contract is explicit. Configuration, rolling logs, and the session marker all live inside the program directory, with no `AppData`, `ProgramData`, or `Roaming` write anywhere in the runtime. Moving or copying the program folder keeps the application fully functional because every runtime path is derived from the executable location. The single exception is the current-user auto-start registry entry, which stores one absolute executable path and would point at a stale location after a move.

A duplicate launch now produces visible feedback instead of a silent exit. The second process posts the registered `TrueTickAnotherInstance` message to the running instance window found through the `TrueTickTrayClass` window class, shows a brief informational dialog titled `True Tick` with the text `True Tick is already running.`, and exits with code 0. The running instance handles that message by recording `lifecycle.another_instance_attempt` with `source=second_launch`. Registering the message, finding the window, and posting it are each recorded with bounded native results, and a missing running window or a zero message identifier skips the notification without failing the exit.

The portable launcher reports a repair condition through a native surface in addition to stderr. A missing or invalid `active-slot.txt`, a missing slot payload, or a launch failure produces one bounded repair reason that carries the cause and the package root, prints it to stderr, and shows a native error dialog titled `True Tick launcher`. The launcher still exits with a failure code and never silently selects slot A.

The launcher performs Tier 2 autonomous slot-to-slot rollback. When the active slot fails executable existence, file shape, or `SHA256SUMS.txt` manifest checksum verification, the launcher verifies the alternate standby slot against the same manifest. A valid standby triggers an autonomous rollback decision record appended to `Data/logs/launcher-rollback.log`, an `active-slot.txt` rewrite to the standby slot, and a clean boot of the standby payload. When both slots fail verification the launcher returns a `BothSlotsCorrupted` repair condition naming both failure causes and requires package repair.

Submenu anchoring is deterministic. The root popup, the Schedule submenu, and the Status submenu use `TPM_LEFTALIGN | TPM_TOPALIGN` together with `TPM_RIGHTBUTTON | TPM_RETURNCMD`, and the cursor anchor is clamped to the primary work area before the popup opens, so a submenu opens consistently to the side of and below its parent item instead of flipping to an unexpected position. A failed `GetCursorPos` still records `native.GetCursorPos.error` and aborts the popup.
