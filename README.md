...
Rows are chronological retained session rows in the current bounded snapshot. Earlier rows may no longer exist after diagnostic retention truncation. Empty input and `all` select every retained row. `12` selects one visible report row. `12-24` selects inclusive 1-based visible report rows. Whitespace is trimmed. Malformed, reversed, zero, negative, and out-of-range values show a short validation message, disable both actions, and create a diagnostic validation event. The grid and actions use the same current bounded snapshot.

`Copy` creates spreadsheet-friendly UTF-16 Windows clipboard text as TSV with the ten report columns `Sequence`, `Elapsed`, `Operation`, `Parent`, `Correlation`, `Phase`, `Source`, `Outcome`, `Event`, and `Details`. The clipboard path uses standard Windows ownership transfer and releases temporary allocations safely. `Export` opens the standard Windows Save dialog with the concise default filename `true-tick-log.tsv`, then writes a UTF-8 TSV file with the same header and selected rows through a bounded same-directory temporary file replacement. Canceling the dialog is a normal logged cancellation and is not shown as an error.
