...
The `True™ Tick Status and Diagnostics` window places a compact toolbar between the status summary and the read-only Logs report. It has a `Rows:` input, `Copy` button, and `Export` button. The window uses explicit native child controls and a DPI-aware layout, so the toolbar remains usable at normal DPI and when the taskbar window is resized.

Rows are retained-session row positions in the current bounded snapshot, not event sequence IDs. Row 1 is the oldest row currently retained and the final row is the newest retained row. Empty input and `all` select every retained row. `12` selects one row. `12-24` selects inclusive 1-based rows. For example, retained rows `353-354` can contain event sequences `753-754`. Whitespace is trimmed. Malformed, reversed, zero, negative, and out-of-range values show a short validation message, disable both actions, and create a diagnostic validation event. When the 512-event cap truncates the session, row positions refer only to the retained newest snapshot and earlier rows no longer exist. The grid and actions use the same current bounded snapshot. A refresh preserves selected events by sequence when they remain retained. If they are gone, the selection is visibly reset and recorded.

`Copy` and `Export` use exactly the selected retained rows and include the eleven report columns `Row`, `Sequence`, `Elapsed`, `Operation`, `Parent`, `Correlation`, `Phase`, `Source`, `Outcome`, `Event`, and `Details`. The clipboard path uses standard Windows ownership transfer and releases temporary allocations safely. `Export` opens the standard Windows Save dialog with the concise default filename `true-tick-log.tsv`, then writes a UTF-8 TSV file with the same header and selected rows through a bounded same-directory temporary file replacement. Canceling the dialog is a normal logged cancellation and is not shown as an error.
