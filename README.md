...
the grid to show only matching events. A `Search:` box sits on the same row and filters
the grid case insensitively over the event name and the details text. An `EN_CHANGE`
notification records `diagnostic.search.changed` with a bounded query byte length. The
search and the category filter compose, so a displayed row satisfies both, and the
`Showing X of Y retained rows` count reflects the composed filter. `Export all` ignores
the search text, the category filter, the grid selection, and the transfer range.
Ctrl+A selects all currently displayed rows when the grid has focus. A refresh preserves selected event sequences when retained. Truncation drops
...
outer request is 1280x520 at 96 DPI. The intended client minimum is 1190x276
logical pixels, including the twelve toolbar controls, the one-row toolbar, and a 96 pixel grid viewport, and
...
list now covers all 18 diagnostic children, including the category filter combo, the
search label, the search input, and the Export all button. `WM_SIZE` positions every control with a direct `SetWindowPos` call and
performs one layout pass. It does not snapshot data, rebuild rows, or fit columns. Horizontal
...
horizontal scroll position when Windows permits it. Smart auto-scrolling pins to the bottom when at the tail of the log, while preserving the user viewport position when scrolled up. Reopening reuses the existing
window and posts one refresh rather than rebuilding synchronously.

The diagnostic summary is a restrained native HUD. A lifecycle-derived State line is larger and bold, followed by a compact five-line telemetry block with `|` separators that carries Effective timing, Ownership, Power, Startup, Running duration, Next action, the retained event history and visible row count, the event-chain integrity status, the operating slot with the resolved portable root, and an About identity line with the branded product name, the Cargo package version, the creator, and the GitHub link. The integrity field runs `verify_event_chain` over the current snapshot and shows `Chain: OK` when every entry hash and link verifies, or `Chain: Error at #<index>` naming the first offending retained row. Native etched separators make the HUD, one-row toolbar, and report grid distinct. The HUD is a read-only `STATIC` presentation with no `WS_VSCROLL`, `ES_AUTOVSCROLL`, or multiline edit style. Its base height is 140 logical pixels. At 96 DPI the toolbar client minimum is 1190 pixels and the minimum client height is 276 pixels. `WM_GETMINMAXINFO` converts those intended client minimums to outer tracking dimensions with `AdjustWindowRectExForDpi`, with the legacy frame API as a fallback. Completing a redraw pass invalidates and updates all 18 diagnostic child controls explicitly, so no control stays blank after `WM_SETREDRAW` while it waits for a mouse hover.

Diagnostic control layout positions each child with a direct `SetWindowPos` call instead of a `BeginDeferWindowPos` batch. Each pass checks `IsWindow` and a positive rectangle per control, records a bounded `diagnostic.layout.skipped` reason when a control is skipped, and records `diagnostic.layout.failure` with the `SetWindowPos` stage and raw Win32 status for a hard failure. Direct positioning eliminates the recurring `ERROR_INVALID_PARAMETER` layout errors that the deferred batch produced. Child creation returns `-1` after every partial child set is destroyed. Reuse verifies the parent and every required child before showing the window.
...
it. The compact tray menu starts with a clickable `True™ Tick v<version>` header sourced from Cargo package metadata. Root menu order is `True™ Tick v<version>`, a separator, `Start`, `Stop`, `Schedule >`, `Settings >`, `Status >`, a separator, `Logs`, a separator, and `Quit`. The root popup, the Schedule submenu, the Settings submenu, and the Status submenu are tracked with `TPM_LEFTALIGN | TPM_TOPALIGN | TPM_RIGHTBUTTON | TPM_RETURNCMD`, and the cursor anchor that seeds the popup is clamped to the primary work area before the popup opens, so submenus open consistently to the side of and below their parent item at any screen-edge position. Start and stop are primary and stay at the top. Scheduling and pausing share `Schedule >`, settings toggles share `Settings >`, and status telemetry shares `Status >`. `Settings >` contains `Auto-start: On/Off`, `Auto-time: On/Off`, and `Auto-resume on AC: On/Off`. Auto-start controls launch at Windows login. Auto-time controls automatic timer acquisition on AC power. Auto-resume on AC reacquires timing when returning to AC power after a restrictive power state released an active session. The current defaults are `startup_enabled = true`, `automatic = false`, and `auto_resume_on_ac = true`. Toggling any item inside Settings keeps the Settings submenu open at the anchor position. The `Schedule >` submenu contains, in order, `Start in >`, `Stop in >`, `Pause for >`, `Interval presets...`, a separator, `Cancel scheduled action`, and `Resume now`. The native menu builder, the internal label vector, and its tests all use `Pause for >`, so the label has one source of truth. Pause offers 5 minutes, 15 minutes, 30 minutes, and 1 hour by default. Schedule offers Start in, Stop in, Pause for, `Interval presets...`, Cancel scheduled action, and `Resume now` when a pause is active. Start and stop schedule choices offer 1 minute, 5 minutes, 15 minutes, 30 minutes, and 1 hour by default. `Interval presets...` opens a native Time Manager dialog window (`TrueTickPresetsClass`) to add, delete, and reset custom interval presets (from 10 seconds to 24 hours), which update the submenus dynamically and persist in `schedule_presets_seconds` in `true-tick.toml`. Tooltips for Schedule actions explicitly state that selecting a new choice replaces any currently pending scheduled action. There are no two scheduling surfaces, stacking, or indefinite pause.
...
The native menu keeps Start, Stop, and Settings toggles open after successful or failed handling. Toggling a setting reopens the Settings submenu at the captured anchor. Highlighting a command uses a standard Windows tooltip. Descriptions cover Duration, Start in, Stop in, Pause for, Cancel scheduled action, Settings, Status, Logs, Start, Stop, auto-start, auto-time, auto-resume, and Quit. The tooltip is destroyed when the popup closes and never changes timer state. Logs closes the menu when it opens diagnostics. Quit is dispatched from the returned `TPM_RETURNCMD` command. Tray left-button-up and right-button-up notifications open this same menu. Button-down and double-click notifications are ignored. Start and Stop remain manual controls and use the same guarded policy and ownership lifecycle as automatic activation. Start remains yellow through query, request, and verification, then turns green only after a verified request. Quit logs its request, active-state decision, dialog result, cleanup result, and exit permission. If timing is running, starting, stopping, pending, degraded, unverified, or ownership is uncertain, the warning offers exactly `Cancel` and `Stop and Quit`. Quitting while running, transitioning, degraded, unverified, or uncertain must confirm through this dialog, must complete cleanup before exit, and must keep the app open when cleanup fails. A failed or uncertain release keeps the app open and allows retry. The diagnostic window shows a bounded local in-memory session log.
...
`True™ Tick: Warning`, and `True™ Tick: Error`. A policy block names its power
reason as `True™ Tick: Blocked (Battery)`, `True™ Tick: Blocked (Battery Saver)`,
or `True™ Tick: Blocked (Power unknown)`, and keeps the generic
`True™ Tick: Warning` only when no power reason applies. Timing is shown as exact
...
root, or to `logs/` beside the executable in a non-portable layout. The first append
to a daily file writes the full 13-column header
`Row,Sequence,Elapsed,Operation,Parent,Correlation,Phase,Source,Outcome,Event,Details,PrevHash,EntryHash`
exactly once per file, which includes a pre-existing yet empty file, so every daily log
is self describing. Each line is
...
required and never silently selects A. The launcher reports that repair condition on a
native surface as well as on stderr. A missing or invalid `active-slot.txt`, a missing
or non-file slot payload, or a failed slot launch produces one bounded repair reason
that carries the cause and the package root, prints it to stderr, shows a native error
dialog titled `True Tick launcher`, and exits with a failure code. Secure signatures and rollback are not
...
The HUD State value is derived from the same lifecycle state used by the tray, not from a stale raw status field. The HUD also names the operating slot (`A`, `B`, or `Development`) and the resolved portable root, and it carries an About identity line with the branded product name, the package version, the creator, and the GitHub link. The history field reports the actual retained event count, the 512 event cap, and whether a truncation marker is present. The grid retains all bounded raw details that fit the current snapshot.

True Tick is self contained by contract. Configuration, rolling logs, and the session marker all live inside the program directory, with no `AppData`, `ProgramData`, or `Roaming` write anywhere in the runtime. Moving or copying the program folder keeps the application fully functional because every runtime path is derived from the executable location. The single exception is the current-user auto-start registry value, which stores one absolute executable path and would go stale after a move until auto-start is toggled again.

A duplicate launch is not silent. The second process posts the registered `TrueTickAnotherInstance` message to the running instance, shows a brief informational dialog titled `True Tick` with the text `True Tick is already running.`, and exits with code 0. The running instance records `lifecycle.another_instance_attempt` with `source=second_launch`. The same startup path also reads `Data/logs/session.state`, which holds `running` while the process lives and `clean` when the shutdown path completes. A previous `running` marker, or any unrecognized content, records `lifecycle.unclean_shutdown` and shows a non-blocking warning that the last shutdown was not clean. A clean prior exit or a first run records `lifecycle.clean_start`. Marker write failures record `lifecycle.session_marker.error` and never change timer state.
